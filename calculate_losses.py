#Frechet inception distance taken from https://github.com/hukkelas/pytorch-frechet-inception-distance/blob/master/fid.py
...
def inception_score(images, batch_size=4):
    #taken from https://github.com/809253928/inception-score-pytorch/blob/master/inception_score.py
    cuda= False
    if cuda:
...
def calculate_losses(args):
    ...
    #calculate segmentation losses as well
        print("Segmentation losses are also being calculated.")
        labels = load_images(args.orig_label_path)
        preds = load_images(args.pred_path)
        l1= l1_loss(labels,preds)
        l2= l2_loss(labels,preds)
        print("L1 loss on segmentation prediction is: ", l1)
        print("L2 loss on segmentation prediction is: ", l2)


if __name__ == "__main__":

    ''' python calculate_losses.py --orig_img_path /cluster/scratch/himeva/DL_Project/data/leftImg8bit/ourtest/frankfurt
     --rec_path /cluster/scratch/himeva/DL_Project/data/leftImg8bit/ourtest/frankfurt -b 4 '''
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=str,
        default="/cluster/scratch/himeva/DL_Project/data/leftImg8bit/ourtest/frankfurt",
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=str,
        default="/cluster/scratch/himeva/DL_Project/data/gtFine/ourtest/frankfurt",
        help="Path to the original labels",
    )

    parser.add_argument(
        "--rec_path", nargs="?", type=str, default=None, help="Path to the reconstructions made by network"
    )
    parser.add_argument(
        "--pred_path", nargs="?", type=str, default=None, help="Path to the predictions made for segmentation by network"
    )
    parser.add_argument("-b", "--batch_size", dest="batch_size",
                      help="Set batch size to use for InceptionV3 network",
                      type=int)
    args = parser.parse_args()
    calculate_losses(args)
